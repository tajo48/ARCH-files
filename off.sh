...
case "$chosen" in
  ...
  Pacman) update=$(sudo pacman -Qu | awk '{print $1}') && install=$(sudo pacman -Ss | grep "^[A-Za-z]" | tr "/" " " | awk '{ print $2}' ) && chosen=$(echo -e $install"\nProgramy:\n"$update | dmenu -i ) && sudo pacman -S --noconfirm $chosen ;;
...
#update=$(sudo pacman -Qu | awk '{print $1}') && install=$(sudo pacman -Ss | grep "^[A-Za-z]" | tr "/" " " | awk '{ print $2}' ) && chosen=$(echo -e $install"\nProgramy:\n"$update | dmenu -i ) && sudo pacman -S --noconfirm $chosen ;;
